...
newsRouter.get('/', async (req, res) => {
  ...
  const news = await db.getAllNews(limit, offset, category);
  res.json({
    news,
    count: news.length,
    timestamp: new Date().toISOString(),
...
// GET macro news only
newsRouter.get('/macro', async (req, res) => {
  const limit = parseInt(req.query.limit as string) || 30;
  const news = await db.getAllNews(limit, 0, 'MACRO');
  res.json({ news, count: news.length });
});

// GET micro news only
newsRouter.get('/micro', async (req, res) => {
  const limit = parseInt(req.query.limit as string) || 30;
  const news = await db.getAllNews(limit, 0, 'MICRO');
  res.json({ news, count: news.length });
});

// GET news by event id
newsRouter.get('/event/:id', async (req, res) => {
  const eventId = req.params.id;
  const news = await db.getNewsByEventId(eventId);
  res.json({ news, count: news.length, event_id: eventId });
});

// GET single news item
newsRouter.get('/:id', async (req, res) => {
  const item = await db.getNewsById(req.params.id);
  if (!item) {
    res.status(404).json({ error: 'News item not found.' });
    return;
  }
  const event = item.event_id ? await db.getEventById(item.event_id) : null;
